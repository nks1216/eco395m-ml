...
if __name__ == "__main__":
    ...
    ct = ColumnTransformer(
        [
            ("cat_preprocess", OneHotEncoder(sparse_output=False), CAT_VARS),
            ("num_preprocess", StandardScaler(), NUM_VARS)
        ]

    )
    model = make_pipeline(ct, LinearRegression()).set_output(transform="pandas")
